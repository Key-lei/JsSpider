...
////////////////////////////////////////////////////
function zjy(params) {
    var methon=params.method;
    var object=JSON.parse(params.object);
    var appid=params.appid;
    var consts=params.consts;
    var datas=params.data;
    const askXpeqqhRw4 = consts[0];
    const asieW57RvXov = consts[1];
    const ackDfab8NESq = consts[2];
    const aciTr2fKlB76 = consts[3];
    const dskqEOsIPbn8 = consts[4];
    const dsilc3CNbGvL = consts[5];
    const dckXRbMn52kW = consts[6];
    const dciuPpemWSdH = consts[7];
    const aes_local_key = consts[8];
    const aes_local_iv = consts[9];
    if(!datas){
        return((function(method, obj, appid) {
            var appId = appid;
            console.log(appId)
            var clienttype = 'WEB';
            ...
            param = AES.encrypt(param, ackDfab8NESq, aciTr2fKlB76);
            console.log(param);
            return param
        })(methon,object,appid));
    }else if(datas)
    {
        return((function(data) {
            data = AES.decrypt(data, askXpeqqhRw4, asieW57RvXov);
            data = DES.decrypt(data, dskqEOsIPbn8, dsilc3CNbGvL);
...
////////////////////////////////////////////////////
// const askXpeqqhRw4 = "awNJkHlARuPPfhNc";
// const asieW57RvXov = "bzFZv5wgMhcsenXB";
// const ackDfab8NESq = "dr5tsu80aLXiYq04";
// const aciTr2fKlB76 = "fy3ox4aEVKg5Sv9d";
// const dskqEOsIPbn8 = "hDNAs3WTtRhEzGlx";
// const dsilc3CNbGvL = "x4SGu4BdyRJWNVrT";
// const dckXRbMn52kW = "oCJzpAOERoMf55G9";
// const dciuPpemWSdH = "pbyUAIG3XAWNDlsX";
...
function dGBBXMzLzYIck5QpSGpB(data) {
    data = AES.decrypt(data, askXpeqqhRw4, asieW57RvXov);
    data = DES.decrypt(data, dskqEOsIPbn8, dsilc3CNbGvL);
    data = BASE64.decrypt(data);
    return data
}
var p3rW5o9GKqYc = (function() {
        function ObjectSort(obj) {
            var newObject = {};
            Object.keys(obj).sort().map(function(key) {
                newObject[key] = obj[key]
        ...
        return function(method, obj) {
            var appId = '891d9782ccc52c93f1519f329dd739e0';
            var clienttype = 'WEB';
            var timestamp = new Date().getTime();
            var param = {
                appId: appId,
...
function s2SbilSwhO1PIj4(method, object, callback, period) {
    const key = hex_md5(method + JSON.stringify(object));
    const data = getDataFromLocalStorage(key, period);
    if (!data) {
        var param = p3rW5o9GKqYc(method, object);
        $.ajax({
            ...
            success: function(data) {
                data = dGBBXMzLzYIck5QpSGpB(data);
                obj = JSON.parse(data);
                if (obj.success) {
                    if (period > 0) {
...
function aqistudyencryptAES(appid,object,consts) {
    var methon='GETDATA';
    ...
    if(!datas){
        return((function(method, obj, appid) {
            ...
            param = BASE64.encrypt(JSON.stringify(param));
            param = AES.encrypt(param, ackDfab8NESq, aciTr2fKlB76);
            // param = DES.encrypt(param, dckXRbMn52kW, dciuPpemWSdH)
            // console.log(param);
            return param
        })(methon,object,appid));
    }else if(datas)
...
function aqistudyencryptDES(appid,object,consts) {
    var methon='GETDATA';
    var object=JSON.parse(object);
    var appid=appid;
    var consts=consts;
...
function aqistudyencryptBASE(appid,object,consts) {
    var methon='GETDATA';
    var object=JSON.parse(object);
    var appid=appid;
    var consts=consts;
...
function aqistudydecrypt(data,as1,as2,ds1,ds2) {
    data = AES.decrypt(data, as1, as2);
    data = DES.decrypt(data, ds1, ds2);
    data = BASE64.decrypt(data);
    return data
}
